...
void *reachability(void *hnd, int size, c4snet_data_t *a, c4snet_data_t *b, c4snet_data_t *c, int B, int C, int found) {
    ...
    if (sizeOfCycle != -1) {
        ...
        if (hasEdge(R, *target) && possibleFind.pt1.x != -1) {
            printf("found at (%f, %f) to (%f, %f) from (%f, %f) to (%f, %f)\n", possibleFind.pt1.x, possibleFind.pt1.y, possibleFind.pt2.x, possibleFind.pt2.y, lastNode->interval.pt1.x, lastNode->interval.pt1.y, lastNode->interval.pt2.x, lastNode->interval.pt2.y);
            found = 1;
            ...
            if (l1.pt1.x != -1) {
                //printf("1edge (%f, %f) to (%f, %f) is reachable from (%f, %f) to (%f, %f) from node[%d]\n", R.e.e1.pt1.x, R.e.e1.pt1.y, R.e.e1.pt2.x, R.e.e1.pt2.y, l1.pt1.x, l1.pt1.y, l1.pt2.x, l1.pt2.y, lastNode->i);
            ...
            if (l2.pt1.x != -1) {
                //printf("2edge (%f, %f) to (%f, %f) is reachable from (%f, %f) to (%f, %f) from node[%d]\n", R.e.e2.pt1.x, R.e.e2.pt1.y, R.e.e2.pt2.x, R.e.e2.pt2.y, l2.pt1.x, l2.pt1.y, l2.pt2.x, l2.pt2.y, lastNode->i);
                lastNode->split = lastNode->split + 1;
                struct node* thisNode = NULL;
                thisNode = malloc(sizeof(struct node));
                thisNode->i = lastNode->i + 1;
            ...
            if (l3.pt1.x != -1) {
                //printf("3edge (%f, %f) to (%f, %f) is reachable from (%f, %f) to (%f, %f) from node[%d]\n", R.e.e3.pt1.x, R.e.e3.pt1.y, R.e.e3.pt2.x, R.e.e3.pt2.y, l3.pt1.x, l3.pt1.y, l3.pt2.x, l3.pt2.y, lastNode->i);
                lastNode->split = lastNode->split + 1;
                struct node* thisNode = NULL;
                thisNode = malloc(sizeof(struct node));
                thisNode->i = lastNode->i + 1;
            ...
            if (l4.pt1.x != -1) {
                //printf("4edge (%f, %f) to (%f, %f) is reachable from (%f, %f) to (%f, %f) from node[%d]\n", R.e.e4.pt1.x, R.e.e4.pt1.y, R.e.e4.pt2.x, R.e.e4.pt2.y, l4.pt1.x, l4.pt1.y, l4.pt2.x, l4.pt2.y, lastNode->i);
                lastNode->split = lastNode->split + 1;
                struct node* thisNode = NULL;
                thisNode = malloc(sizeof(struct node));
                thisNode->i = lastNode->i + 1;
    ...
    if (found == 1 || B == 1 || C == 1) {
		...
		C4SNetFree(c);
            ...
            if (thisNode->split < 2) {
                ...
                thisNode->split = thisNode->split - 1;
            }
        }
